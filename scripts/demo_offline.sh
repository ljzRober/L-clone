...
export BRAIN_DB_PATH="$(mktemp -d)/lclone_demo.db"

echo "=== 1. 初始化数据库 ==="
python3 -m lclone init

echo ""
echo "=== 2. 注册项目 + 同步 spec 索引 ==="
python3 -m lclone proj add demo examples/demo_project --charter "示例项目"
...
echo ""
echo "=== 6. 回顾环: 检索记忆 ==="
python3 -m lclone recall "FastAPI 边界" --project demo

echo ""
echo "=== 7. 规范环: 边界监督 ==="
python3 -m lclone supervise "把数据库换成 PostgreSQL" --project demo

echo ""
echo "=== 8. 回顾环: 问答 ==="
python3 -m lclone ask "我们项目定了什么? 有什么边界?" --project demo
