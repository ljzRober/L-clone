...
$env:BRAIN_LLM = "dummy"
$env:BRAIN_DB_PATH = Join-Path $env:TEMP "brain_demo.db"
...
Write-Host "`n=== 3. 主动记忆 (C: 你说算, 直接生效) ===" -ForegroundColor Cyan
python -m brain remember "后端用 FastAPI, 边界: 单用户" --project demo

Write-Host "`n=== 4. 自动捕获 (B: 进草稿, 待确认) ===" -ForegroundColor Cyan
python -m brain capture "讨论后确定: 6月1日上线; 部署用 Docker Compose" --project demo

Write-Host "`n=== 5. 批量确认草稿 ===" -ForegroundColor Cyan
python -m brain review --all keep

Write-Host "`n=== 6. 回顾环: 检索记忆 ===" -ForegroundColor Cyan
python -m brain recall "FastAPI 边界" --project demo
...
python -m brain ask "我们项目定了什么? 有什么边界?" --project demo
