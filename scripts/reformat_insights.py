#!/usr/bin/env python3
"""批量把 insight 重排成三段卡(要点|背景|影响)。行式 `id || 三段卡` 输出+解析, 稳定且快。
...
def extract_lines(raw: str):
    """把模型输出按行切, 每行解析出 (id, 三段卡)。容错: 去掉 json/代码块围栏。"""
    text = raw.strip()
...
def main() -> None:
    ...
    for start in range(0, len(ins), args.batch):
        ...
        head = "以下每条一行 `id || 内容`。请把每条改写为统一三段卡(保持原意只补结构):\n" \
               "「要点(结论)｜背景/为什么｜影响/以后注意」, 自然一段(2-4 句), 中文。\n" \
               "若原文指向某个进化资产/契约/源文件, 就在提到它的那句话里就地写 [[evo:名字]] / [[spec:标识]] / [[src:路径]];\n" \
               "不要另起一行做引用列表。去掉旧模板里的「归属」段(该段已废弃)。\n" \
               "输出格式: 每条一行 `id || 三段卡`, 不要 JSON。\n\n"
        ...
        for r in chunk:
            ...
            if not card or len(card) < 20:
                # 该条没被模型改到 -> 降级单条重试一次
                p2 = ("把下面 insight 改写成统一三段卡(要点|背景|影响), 2-4句, 只输出一行 `id || 三段卡`, 中文, 不要 JSON。\n\n"
                      f"{r['id']} || {r['content']}")
