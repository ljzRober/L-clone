...
- **通用进化资产** SHALL 只包含**数据与可执行产物**（当前为 `lclone/data/starter/scripts/` 下的示例脚本）；**说明性文档 SHALL NOT 进种子**（规范/说明/模板/示例一律走 insight，见 `memory-capture` > 分工边界）。进化资产 SHALL 走**版本化发布**写入进化资产库（详见 `evolution-store`），并物化到记忆区进化缓存目录（默认 `~/.lclone/evolution/`，可用 `LCLONE_EVO_DIR` 覆盖——该目录只是可复现缓存，容器化部署无需持久化）。种子 SHALL NOT 绕开版本库直接写文件。
...
THEN 写入 4 条全局通用洞察与通用进化资产（数据/脚本类），并报告种入条数
...
WHEN 查看种子目录
THEN `lclone/data/starter/evolutions/` 不含任何说明性文档（规范/说明/模板不进 evolution，也不进种子）
