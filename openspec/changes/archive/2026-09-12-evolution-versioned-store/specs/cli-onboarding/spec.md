...
- **通用进化文件**（洞察格式模型 / 记忆准入标准 / 会话归属与落库约定 + 示例脚本），SHALL 走**版本化发布**写入进化资产库（详见 `evolution-store`），并物化到记忆区进化缓存目录（默认 `~/.lclone/evolutions/`，可用 `LCLONE_EVO_DIR` 覆盖——容器化部署指向持久化卷，因为该目录只是可复现缓存）。种子 SHALL NOT 绕开版本库直接写文件。
