...
拆成两块(对应两条接入命令):
  check_backend     后端自检: .env / provider / 数据库 / LLM 连通 (空项目为正常态)
  check_integration 前端集成自检: skill / DSH 插件 / Claude Code hooks / Codex hooks / commit 钩子
  check_all         = 两者合并(向后兼容)
"""
...
def check_backend(db_path: Optional[str] = None, check_llm: bool = False) -> List[dict]:
    """后端自检: 只查本机服务相关的 .env / provider / 数据库 / 项目 / LLM。"""
    ...
    add("provider 预设", bool(pname), pname or "未识别",
        "" if pname else "BRAIN_BASE_URL 未匹配任何预设, 可用 lclone setup 重配")
...
def check_integration(home: Optional[Path] = None) -> List[dict]:
    """前端集成自检: 只查 skill / 插件 / hooks, 不碰后端配置。"""
...
def check_all(db_path: Optional[str] = None, home: Optional[Path] = None,
    ...
    """合并: 后端 + 前端(向后兼容)。"""
    return (check_backend(db_path=db_path, check_llm=check_llm)
            + check_integration(home=home))


def _marks() -> tuple:
    """返回 (ok, bad) 标记; 控制台无法编码 emoji 时回退 ASCII。"""
...
def render_sections(sections) -> str:
    """分组自检: sections = [(标题, [检查项...]), ...], 用于前后端分段展示。"""
    ok_mark, bad_mark = _marks()
    lines = ["L-clone 自检:"]
    total_ok = total = 0
    for title, items in sections:
        ...
        for r in items:
            ...
            if r.get("hint"):
                lines.append(f"     ↳ {r['hint']}")
    lines.append("")
    lines.append(f"通过 {total_ok}/{total}")
    if total_ok < total:
        lines.append("按上面 ↳ 的提示修复, 再重跑 lclone doctor")
    return "\n".join(lines)
