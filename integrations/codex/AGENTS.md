...
- **对话中**：出现确定信息（决定了 X / 边界是 Y / 上线时间 / 值得记的事实）时，调 `lclone capture "<内容>"` 沉淀为草稿（B 类，用户 review 确认），并在回复里提示草稿号。
- **用户明确说"记住/记一下"**：调 `lclone remember "<内容>"`（C 类）。记录(`--level note`)直接生效；决策默认进待确认，用户当场已确认该决策时加 `--confirmed` 直接生效。
- **归属**：优先按当前 git 仓库匹配已注册项目（`lclone proj list` 查看）；跨项目/个人偏好落全局层；拿不准问用户。
- **分工**：改变项目 spec（需求/场景/边界）走 sp-spec（openspec）；脑内记忆（决策/记录/charter）走 lclone。
