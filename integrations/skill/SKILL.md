...
1b. **会话进行中（每轮，硬性）**：后台捕获（DSH 插件 / CLI hooks）会持续静默把决策写进待确认。**每轮回复用户之前，先调一次 `bootstrap`（query 可为空）检查【待确认决策】；有就立刻用 `ask_user_question` 逐条请用户保留/删除，不要等会话结束或下个会话**——否则决策会一直挂在 pending 无人处理。这就是「在对话中推一条选择消息」，DSH 与 CLI 都适用（由 agent 生成对话消息，不依赖浏览器 UI）。
...
4. **模块归属（代码强制）**：module 由 LLM 分类到代码维护的词表（复用已有名或给粗粒度新关注点），代码归一化/复用/防泛名；**记录(note) 无模块**，落在项目层（偶尔全局）。你无需也不应手动指定 module，除非用户明确点名。
