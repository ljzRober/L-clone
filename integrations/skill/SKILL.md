...
1b. **会话进行中（每轮，硬性）**：后台 DSH 插件会持续静默捕获决策进待确认。**每轮回复用户之前，先调一次 `bootstrap`（query 可为空）检查【待确认决策】；有就立刻用 `ask_user_question` 逐条请用户保留/删除，不要等会话结束或下个会话**——否则决策会一直挂在 pending 无人处理。
