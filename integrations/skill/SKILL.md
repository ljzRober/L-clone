...
3. **归属判定（优先级：git → 问用户）**：
   a. **先按 git 定项目（优先）**：确定当前对话/工作所在的 git 仓库（`git -C <目录> rev-parse --show-toplevel`），匹配 lclone 已注册项目；匹配到就用它——调用 `remember`/`capture` 时传 `project=<项目名>`，或传 `cwd=<仓库目录>` 让服务器自动检测。
   b. **没有 git 或匹配不到 → 必须问用户**：是否新建 project（请用户取名）还是上升到全局层（个人区）。不要静默默认全局。
   c. **自动归模块**（项目内次级竖向，与 sp-spec 分 spec 同逻辑）：确定项目后，按工作主题给 module（按关注点拆，如 web/server/memory-capture/dsh-plugin/cli/deploy）；拿不准留空，由分类器自动派生。
   d. **拿不准就问用户**：不确定归属哪个项目时**问用户，不要猜**。
