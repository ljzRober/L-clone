...
def extract_memories(text: str,
    ...
    自筛: 提示词要求 LLM 只提炼「真正值得长期记住」的高价值内容, 宁可少提甚至不提
    (过程性琐碎/一次性/显而易见/临时性/代码改动一律不输出), 减少对用户的打扰。
    ...
    prompt = (
        "你是一个记忆筛选器。下面是工作/讨论记录, 请只提炼「真正值得长期记住」的内容。\n"
        "提炼前先自问: 用户下次做相关决策时还用得到吗? 值得跨会话保留吗?\n"
        "只有高价值的才输出——选型/约定/边界/关键事实。\n"
        "以下一律不要输出(宁可少提甚至不提): 过程性琐碎、一次性操作、显而易见的内容、临时性内容,\n"
        "以及代码改动/接口变化/新增端点/重构/修 bug (这些归 git 和 spec)。\n"
        "整段没有值得记的就输出空, 不要凑数。\n"
        "每条一行, 格式: 类型[模块]: 内容\n"
        "- 类型: decision(选了方案/定了规则/约定边界, 需用户确认) 或 note(值得记的过程事实/观察)\n"
        f"- 模块: 英文短名, 优先复用已有模块 ({mod_hint}); 新关注点才起粗粒度名 (如 web/server/cli/memory-capture)。\n"
