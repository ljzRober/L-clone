...
| 说明性内容: 规范/准则/理由/观察/经验/模板/示例 | lclone **洞察(insight)** —— 可召回, 注入正文须自足 |
| 数据与可执行产物: 脚本/工具/配置/密钥表 | lclone **进化资产(evolution)** —— 版本化内容库, `[[evo:]]` 指向 |

**引用纪律**:一条洞察如果明确对应仓库内某具体 spec/源文件/进化资产,项目级记忆可把
`[[evo:名字]]`/`[[spec:id]]`/`[[src:path]]` **就地写在提到它的那句话里**(link, not copy,
权威内容留在仓库),不另起一行做引用列表;全局级记忆无仓库上下文,不标此类链接(只有 `[[m:N]]`)。
一条记忆**升格为硬契约**时进 sp-spec,并写入 `[[spec:id]]` 引用或删除,避免双份漂移。

## 3. 洞察 (insight): 原子化富知识卡

**洞察 = 一条原子化、自包含、内容丰富的知识/见解/教训**——每条是一件事
(一个决定 / 一条经验 / 一个观察 / 一条复盘),**按三段卡书写(约 2-4 句)**:
...
指向进化资产/契约/源文件的引用(`[[evo:]]`/`[[spec:]]`/`[[src:]]`)**就地写在提到它的那句话里**,不另起引用行。
存储时整卡做 embedding。**不逐字转录对话/代码**(那是 git/spec),也不压成一行干结论。
太长就拆成多条洞察,一条一个观点。
...
自动捕获先过一道**确定性准入闸门**(`lclone/gate.py`,纯脚本、不调 LLM),把每一轮文本判成三档:

| 档位 | 触发 | 代价 |
| --- | --- | --- |
| `skip` | 有效文本 < 12 字;剥掉代码块/堆栈后无自然语言;命中「做了什么」(修复/重构/迁移/回滚等);疑问句;短句寒暄 | **0 次 LLM 调用** |
| `candidate` | 命中**决定性**的决策/约定/规则或教训/经验说法(如 决定·定了·定为·统一·口径·必须·踩坑·原因在于) | 直接交 LLM 成卡 |
| `uncertain` | 既没命中强信号,也不构成噪声 | 先一次极便宜的 1/0 判定,判「是」才成卡 |

> 词表的**唯一来源**是 `gate.py` 的常量块(上面只是节选);`lclone gate "<文本>"` 可打印真实判定与命中词。

三条硬规则:**只看用户轮**(按 `用户：`/`助手：` 切分——助手输出多为「做了什么」,是最大噪声源;
宿主注入的 skill 全文/记忆区先剥掉,它们不是用户说的话);
**疑问句优先**(以 `？`/`?` 结尾按提问处理,即使含强信号词也不判候选);
**否定作用域**(信号词前 3 字内有 不/没/无需/不用/取消 时该命中不计;不错/不但/不仅/不如/不少 除外);
**单轮上限 1 条**(默认一条都不产出,上限不是配额)。用户显式说「记住 / 记一下」→ 旁路闸门。
词表只此一处,用 `lclone gate "<文本>"` 可自测判定。

成卡之后仍有两道确定性护栏:排除「做了什么」(修复/重构/改接口/修 bug → 归 git 与 spec)、
丢弃过短琐碎(< 4 字),再做向量去重与归一化文本去重(去空白标点后前 80 字相同视为重复)。

**auto 卡的「用户轮证据」**:卡片主张要在用户轮里有出处(至少命中一个特征片段),否则不成卡——
这条拦的是「助手自己排查出来的环境近况」(skill 装在哪、目录有没有 .git、容器里跑代码还是卷)
与通用设计常识:它们跟用户轮只是同主题,却没有一句是用户说的。用户显式说「记一下」的轮次豁免。

**另外三条结构规则**(零 LLM 成本,来源见 `docs/记忆准入门控调研.md`):
**短时性措辞**(这次/今天/目前/临时/`for now`/`today`)在没有决策信号时判 skip——它是可重测的近况,
不是知识(命中决策信号的句子不受此限);**自包含性**——卡片必须含「要点」且至少含「背景」或「影响」,
只有一句结论的残卡不收(离开原始对话读不懂);**引用就地写**——指向进化资产/契约/源文件的引用
(`[[evo:]]`/`[[spec:]]`/`[[src:]]`)必须写在**提到它的那句话里**,不另起一行做引用列表——
同一资产写两遍会渲染出两个一样的引用芯片,召回时也会把同一份内容带出两遍。

**准入质量指标**:`lclone stats [--days 7]`(设了 `LCLONE_WEB_URL` 就查远端,`--local` 切本地)打印
队列精确率(`keep/(keep+delete)`,≥0.5 健康)、每会话复核负担(目标 ≤5)、复用率。
每次确认动作都会写 `review_log` 留痕;**delete 会真删记忆行, 留痕故意不加外键**——
否则精确率永远算不出来。这三个数只用于**准入改动前后对比**,没有公开基准可作绝对标尺。
...
- **改动是显式动作**:`publish` 把本地内容推为新版本(内容未变则不加版本),`publish --all` 一次推全部 dirty。
  **没有自动双向同步,也没有冲突合并** —— 这是 npx/cacache 模型,不是 git 模型:git 下本地是可写工作副本
  (两边都可改→需要 push/pull/merge),这里本地是纯缓存(只有服务器能改→不需要合并)。
- **链接**:insight 用 `[[evo:name.ext]]` 指向某进化资产(insight→evolution),1..N 个 insight 可支撑一个 evolution;
  检索命中 insight 时顺带把该资产带出(取当前版本)。

**首次种子内容**:新装的空库不是空的——`lclone setup` 会在初始化数据库后种入一份**通用**内容:
4 条全局洞察(洞察格式约定 / 记忆与 spec 分工 / 归属判定 / 删除纪律)+ 通用进化文件
(洞察格式模型 / 记忆准入标准 / 会话归属与落库约定 + 2 个示例脚本),让你立刻看到"一条洞察长什么样、进化资产怎么用"。
种子洞察直接 `active`(**不进待确认、不弹窗**)、`source_type='seed'`;已种过的 key 记入 `seed_state`,
**删掉后不会被自动回灌**(尊重你的删除);进化文件已存在则不覆盖,只有 `--force` 才覆盖。
种子进化文件同样走 publish 落进内容库(不是绕开索引直接写文件)。
`--no-seed` 可跳过,`lclone seed [--force] [--dry-run]` 可单独重放。
...
- 召回命中 A 时自动顺藤把 A 链接指向的记忆一起带出(一层、限量,`recall --no-follow` 可关闭);
  会话面召回(MCP recall / `/api/recall` / bootstrap 相关记忆)默认**并入全局层**(全局层在任何会话都加载), 故项目会话也能命中全局的 `[[evo:]]` 指路卡并顺边带出资产正文;
...
| insight 三段卡(非流水账) | 让大家能独立读懂一条洞察, 不依赖上下文; 也不逐字转录(那是 git/spec) |
