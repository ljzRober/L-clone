...
- **分层记忆**:会话流水(L0)→ 决策/重要修改点(L1)→ 项目 spec 索引(L2)
- **回顾环**:新会话自动召回相关记忆,回答"我上次做到哪、定了什么"
- **规范环**:新提议对照项目 spec 的边界条件,输出 ✅通过 / ⚠️警告 / ❌违反 检查报告
- **B/C 记忆写入**:AI 提炼 + 你确认的自动捕获(B);你说算的主动记忆(C);每条可追溯来源
- **竖向分层**:每个项目独立命名空间,具体事务留在仓库,大脑只管方向和决策
- **多端访问**:CLI + Web 面板 + REST API,任何设备浏览器即可用
...
|---|---|
| **Python** | **>= 3.10**(无需 GPU) |
| **第三方库** | `openai`(>=1.30)/ `fastapi`(>=0.110)/ `uvicorn`(>=0.29) |
| **数据库** | SQLite(内置,无需安装) |
...
```powershell
Copy-Item .env.example .env     # 填 OPENAI_API_KEY / BRAIN_BASE_URL / 模型名 (见 docs/CLI.md)
.\.venv\Scripts\python.exe -m lclone web    # 浏览器打开 http://127.0.0.1:8000
```

### 记录与 Claude/AI 的对话

把对话内容粘给大脑,它自动提炼决策(进草稿,你确认后生效):

```powershell
lclone capture "我想做个健身记录工具。Claude 建议: 用 Python + SQLite, 每周自动汇总, 先跑通再优化。我同意, 定于下月 1 号上线。"
lclone review --all keep        # 你过目后确认
lclone recall "健身工具"         # 下次直接想起
```

不想粘长对话?让 Claude 在结尾输出一行结论,`lclone remember "健身工具: Python+SQLite, 每周汇总, 下月1号上线"` 直接入库。
Web 面板「记忆」页签也可粘贴对话。v1 接入 MCP 后无需复制粘贴,自动沉淀。

### 完整自测(无需 API Key)

```bash
python tests/test_offline.py    # 20 项离线自测
```
